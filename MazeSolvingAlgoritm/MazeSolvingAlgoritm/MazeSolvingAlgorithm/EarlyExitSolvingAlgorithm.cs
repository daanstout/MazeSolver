...
namespace MazeSolvingAlgoritm.MazeSolvingAlgorithm {
    public class EarlyExitSolvingAlgorithm : IMazeSolvingAlgorithm {
        Queue<Node> q;
            ...
            if (maze == null)
                ...
                while (!q.isEmpty) {
                    ...
                    if (current.x < maze.width - 1) {
                        if (!maze.isWall(current.x + 1, current.y) && maze.GetNode(current.x + 1, current.y).distance > current.distance + maze.GetNode(current.x + 1, current.y).weight) {
